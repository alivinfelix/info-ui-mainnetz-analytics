...
function parseData(data, oneDayData, twoDayData, oneWeekData, ethPrice, oneDayBlock) {
  ...
  if (data?.token0?.id === '0xd7b21500dff3d5578b21ebf7f25be806087559a7') {
    data.token0.name = 'Netz (Wrapped)'
    data.token0.symbol = 'NETZ'
  }
  if (data?.token1?.id === '0xd7b21500dff3d5578b21ebf7f25be806087559a7') {
    data.token1.name = 'Netz (Wrapped)'
    data.token1.symbol = 'NETZ'
  }
  return data
}
